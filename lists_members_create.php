...
if(isset($_GET["list_id"],$_GET["username"]) && preg_match("/^[0-9]{1,}$/",$_GET["list_id"]) && preg_match("/^[0-9a-zA-Z_]{1,15}$/",$_GET["username"])) 
{ 
 $object = new oauth(); 
 $param["list_id"] = $_GET["list_id"]; 
 $param["screen_name"] = $_GET["username"]; 
 $json = $object->request("POST","lists/members/create",$param); 
 if(empty($json->errors)) 
 { 

?> 
「<?= $json->name ?>」に<?= $json->user->name . "(" . $json->user->screen_name . ")" ?>を追加しました<br> 
<?php 
